...
const navItems: NavItem[] = [
  ...
  { to: '/suppliers', label: 'الموردون', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><path d="M9 22V12h6v10"/></svg>, roles: ['ADMIN', 'HR_COORDINATOR', 'SALES', 'MONITOR'] },
  ...
  { to: '/gps/employee', label: 'لوحة GPS', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/></svg>, roles: ['GPS_ENGINEER'] },
]
    ...
    <SessionContext.Provider value={{ employee, setEmployee, permissions: employeePermissions }}>
      <div dir="ltr" className="flex min-h-screen bg-[#f0f4f9]">
        ...
        <aside dir="rtl" className="sticky top-0 flex h-screen w-[260px] flex-col bg-[#0f2040]" style={{ minWidth: 260 }}>
          ...
          {/* User card */}
          <div className="mx-4 mb-4 flex items-center gap-3 rounded-xl bg-white/5 px-4 py-3">
            <div className="text-right flex-1">
              <p className="text-sm font-bold text-white">{employee.name}</p>
              <p className="text-xs text-blue-300/70">{roleLabels[employee.role]}</p>
            </div>
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#2c5aad] text-sm font-bold text-white">
              {employee.name.charAt(0)}
            </div>
          </div>
