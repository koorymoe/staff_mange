...
func (h *LeaveRequestHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	//
	// ⚠️ بس الطارئة مستثناة — وهذا مو تساهل:
	// الموت والحادث والمرض المفاجئ ما ينتظرون يومين. لو فرضنا
	// المهلة عليها، الموظف الي أبوه بالمستشفى ما يقدر يقدّم طلب
	// أصلاً، فيغيب بلا ورق — ويطلع «بلا بصمة» بسجله وينحاسب على
	// شي مو ذنبه.
	//
	// والقرار يبقى للمدير: النظام يسمح بالتقديم، مو يوافق تلقائياً.
	needsNotice := kind != model.LeaveKindUrgent
	if needsNotice {
		now := time.Now()
