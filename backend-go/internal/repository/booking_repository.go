...
// ListServicePaperworkByKind حجوزات نوع خدمة معيَّن المنجزة — منتقي
// الربط بشاشة فاتورة الخدمة.
//
// (ع): «المفروض تطلعله حجوزات الجي بي اس حته يسوي الهن فواتيرهن،
// ونفس الشي للموظف الثاني الي راح يسوي فواتير الداش كام».
//
...
// ويتوقّع — بحق — إن هذا كل المطلوب. الإعداد الي ما يعرف الموظف
// إنه موجود هو إعداد مكسور.
//
// فالمصدر صار **صلاحية الفاتورة** (يحرسها المسار) + **نوع الخدمة**:
// من عنده `invoice_gps` يشوف حجوزات الجي بي اس، ومن عنده
// `invoice_dashcam` يشوف الداش كام. إعداد واحد، وهو الي يسوّيه أصلاً.
//
// ⚠️ **والمطابقة صارت صريحة** (`serviceKind = $1`) مو «المؤشَّرة أو
// غير المؤشَّرة»: بلا شرط الورق، «غير المؤشَّرة» تعني **كل خدمات
// الشركة** (٢٧ خدمة بالمختبر) — يعني قائمة الداش كام تطلّع حجوزات
// الصباغة والنجارة. وقائمة غلط أسوأ من فاضية. فالخدمة الي ما
// انتأشّر نوعها ما تطلع، **والواجهة تگول هذا بصراحة** وتدل على
// شاشة التأشير.
//
// ⚠️ وهاي تكشف أسماء زبائن حجوزات هذا النوع لصاحب صلاحية فاتورته —
// **وهذا مقصود**: ما يكدر يسوّي فاتورة لزبون بلا ما يعرف منو هو.
// والحصر بالنوع يبقى، فصاحب فاتورة الجي بي اس ما يشوف غير زبائنه.
func (r *BookingRepository) ListServicePaperworkByKind(kind string, limit int) ([]model.Booking, error) {
	if limit <= 0 || limit > 500 {
		limit = 200
	}
	bookings := []model.Booking{}
	// ⚠️ الخدمة تنمسك من عمود الحجز **أو** من جدول الخدمات المتعددة:
	// حجز رئيسيته «كاميرات IP» وفيه داش كام كإضافة لازم يطلع.
	err := r.db.Select(&bookings, `
		SELECT b.* FROM "Booking" b
		WHERE b."archivedAt" IS NULL
		  AND b.status = 'COMPLETED'
		  ...
		  -- 🔴 **وحجوزات الاستيراد التاريخي تنشال** — نفس طلبه.
		  -- وكنت كتلتله إن ماكو علامة تميّزها وطلبت منه تاريخاً
		  -- فاصلاً، **وهذا چان غلطاً**: العلامة موجودة وهي كود
		  -- الحجز نفسه ('OLD-…') الي يحطّه سكربت الاستيراد التاريخي.
		  -- دوّرت على عمود بالجدول وعلى أداة استيراد الجي بي اس (وهي تسوّي
		  -- أجهزة مو حجوزات) وما فتحت سكربتات الجذر.
		  --
		  -- ⚠️ وفواتيرها **تنستورد من دفتر الحسابات** بسكربت
		  -- استيراد الفواتير التاريخية باسم المالك — مو ينكتبن
		  -- بالإيد. فعرضها هنا يعني مسؤول الخدمة يفوترها مرة ثانية
		  -- بسعر يخمّنه، ويزاحم ٢١٩٠ حجزاً شغلاً حقيقياً.
		  --
		  -- ⚠️ والشرط مكتوب هنا صراحةً مو مشتقاً من شرط «الورق مكتمل»:
		  ...
		  -- **يسوّيله فاتورة ثانية** — مبلغ مضاعف بحسابات الشركة،
		  -- وما عنده أي إشارة إنه سوّاها من قبل.
		  --
		  -- ⚠️ والشرط على الحالة مو على أي صف: المسودة الحقيقية
