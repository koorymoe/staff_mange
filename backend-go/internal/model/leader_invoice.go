...
// إداري الكوادر ياما الليدر.» الشغل الداخلي (تركيب كاميرا بمكتبنا،
// شد جي بي اس على سيارة الشركة) ينصرف عليه مواد ووقت كادر مثل أي
// شغل — وبلا فاتورة يضل كلفة ما محسوبة على أي قسم.
//
// ⚠️ **ماكو جدول كلفة ينطي سعراً هنا**: ماكو زبون ولا سعر بيع، فالمبلغ
// **تقدير** يكتبه الي يسوي الفاتورة — نفس آلة الفاتورة اليدوية
// (`PricingModeManual`) بالضبط، وبلا أي عمود جديد.
type CreateInternalInvoiceRequest struct {
	// BookingID **إجباري** — وهو الي يعزل هاي الصلاحية: المعالج
	// يفحص نوع الحجز ويرفض أي شي مو INTERNAL، فصاحب الصلاحية ما
	// يكدر يفوتر شغل زبون بيها.
	BookingID string `json:"bookingId"`
	// Work وصف الشغل — إجباري وبطول معقول، هو المرجع الوحيد للمحاسب.
	Work string `json:"work"`
