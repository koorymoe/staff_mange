...
// ═══ توقيف الخصم التلقائي ═══
//
// DisciplineAutoPenaltyPaused الخصم التلقائي **موقوف مؤقتاً** بقرار
// صاحب النظام: «وقّفها مؤقتاً، بعدين نعدّل عليه ونرجّعه للشغل».
//
// ⚠️ **توقيف لا إلغاء**: القواعد والمهل والمكانس كلها تبقى بمكانها
// وتشتغل وتحسب — الي انوقف هو **تنفيذ** الخصم بس. والقرارات البشرية
// ما تتأثر إطلاقاً: تعديل المالك (`/api/discipline/adjust`) ومخالفة
// المدقّق ببلاغات التدقيق **يبقون شغّالين**.
//
// ⚠️ **وإرجاع النقاط يبقى شغّالاً** (قرار (ع) الصريح) — يعني كل موظف
// يصير «نظيف» تلقائياً ورصيده يرجع لـ100 خلال DisciplineCleanDaysToRestore
// أيام. **السجل ما ينمحي** (صفوف DisciplineEvent تبقى كاملة)، الرصيد بس.
//
// ⚠️⚠️ **قبل ما ترجّعها `false` — اقرأ هذا**:
// المكانس تخصم على **كل** حجز فاتت مهلته، **حتى الي تأخر أثناء فترة
// التوقيف**. فأول ما ترجع، تنزل **دفعة غرامات بأثر رجعي على الكل مرة
// وحدة**. (ع) قرر يحسم هالسؤال **وقت الرجوع مو وقت التوقيف** — فلازم
// يُحسم أولاً: إما نعفي فترة التوقيف بمرشّح تاريخ، أو نقبل الدفعة.
// وسطور السجل («انتخطّت غرامة …») تنطي العدد الحقيقي وقتها.
const DisciplineAutoPenaltyPaused = true
