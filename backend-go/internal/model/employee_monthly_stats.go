...
type EmployeeMonthlyStats struct {
	...
	//
	// طلب (ع): «كل موظف يدخل إجمالي مبالغ أكثر من راتبه». والشاشة
	// چانت تعرض **العمولة** بس — وهي الي ياخذها الموظف، مو الي
	// يدخّله للشركة. فالسؤال الي يقرّر الرواتب چان بلا رقم.
	//
	// RevenueBrought صافي فواتير الليدر الي **رفعها هو** بالشهر.
	// ⚠️ **بلا تكرار**: الفاتورة إلها صاحب واحد، فمجموع الموظفين =
	// مجموع الفواتير بالضبط.
	RevenueBrought float64 `json:"revenueBrought"`
	// Salary راتبه الشهري — ⚠️ **مؤشّر لأنه قابل للفراغ**:
	// موظف بلا راتب مكتوب يعني **ماكو مقارنة**، مو راتب صفر.
	Salary *float64 `json:"salary"`
	// SalaryCoverage نسبة تغطية الراتب = الإيراد ÷ الراتب × ١٠٠.
	// ⚠️ **nil لمّا الراتب فاضي أو صفر** — تُعرض «—» مو ٠٪ ولا ∞.
	// وقرار (ع): **١٠٠٪ تكفي** (يعبر راتبه)، مو أضعافاً.
	SalaryCoverage *float64 `json:"salaryCoverage"`

	// نقاط الكي بي اي — نفس آلية تسجيل النقاط الموجودة أصلاً (KpiEvaluation)،
	// مجموع النقاط غير الملغاة خلال الشهر.
	// KpiPoints مجموع التقييمات اليدوية بس (خصومات ومكافآت المدير).
	// ⚠️ مو نفس النقاط الكاملة — شوف SmartKpiPoints تحت.
	KpiPoints int `json:"kpiPoints"`
	// SmartKpiPoints النقاط الكاملة المحسوبة من الستة عناصر (الحجوزات،
	// السرعة، التقارير، الحضور، الشكاوى، التقييم اليدوي) — هي الرقم
	// الي يقصده المالك لما يسأل «شكد نقاط هذا الموظف؟»، وهي الي
	// يفصّلها المخطط. كان معروض بمكانها مجموع التقييمات اليدوية بس،
	// فيطلع صفر لأغلب الموظفين (أكثرهم ماكو عليهم خصومات).
	SmartKpiPoints int `json:"smartKpiPoints"`
