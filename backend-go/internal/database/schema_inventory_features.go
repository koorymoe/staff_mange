...
func inventoryFeaturesVersionedMigrations() []Migration {
	return []Migration{
		...
		{
			// دور «تقني» — منفصل عن «فني». التقني يتولى أكثر من خدمة وحدة،
			// فينوجّه لحجوزات خدمات مختلفة حسب مهاراته مو حسب تخصص واحد.
			// بلا صلاحيات إضافية افتراضية — تُمنح له من صفحة الصلاحيات.
			//
			// ⚠️ ينضاف هنا مو بقائمة migrations القديمة: تلك مرقّمة حسب
			// موقع العنصر بالمصفوفة، فأي إضافة بوسطها تزحزح كل الي بعدها
			// وتخرب سجل الترحيلات المطبّقة.
			Version: "0195_employee_role_technical",
			SQL: `
				ALTER TYPE "EmployeeRole" ADD VALUE IF NOT EXISTS 'TECHNICAL';
			`,
		},
		{
