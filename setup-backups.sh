...
عندك خياران، اختار واحد على الأقل:
  ...
  ── الخيار ٢: سيرفر أو جهاز ثاني عبر SSH ──
  ضيف بـ.env:
       BACKUP_SSH_TARGET=user@host:/path/backups/
  (لازم مفتاح SSH بلا كلمة سر بين السيرفرين)

  ── الخيار ٣: قرص ثاني على نفس الجهاز ──
  ضيف بـ.env:
       BACKUP_SECOND_DIR=/mnt/backup-disk
  أضعف حماية (نفس الجهاز)، بس أحسن من ولا شي.

  ── الخيار ٤: Hetzner Backups (صورة كاملة للسيرفر) ──
...
if [ -z "${BACKUP_PASSPHRASE:-}" ]; then
  ...
  echo "   ضيف بـ.env سطر تشفير حتى تنشفّر قبل ما ترتفع لأي مكان:"
  echo "       BACKUP_PASSPHRASE=<كلمة-سر-طويلة>"
  echo "   ⚠️ واحتفظ بهاي الكلمة بمكان آمن برّا السيرفر — بدونها النسخ ما تنفتح."
fi
