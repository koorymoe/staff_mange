...
def rest_offsets(rig, src):
    """فرق وضعية الراحة لكل زوج: `restSrc⁻¹ · restRig` (دوران بس)."""
    out, missing = {}, []
    for sname, rname in BONE_MAP.items():
        ...
        if not sb or not rb:
            ...
            continue
        # `matrix_local` وضعية الراحة بفضاء الهيكل
        # 🔴 **نسخ الاتجاه المطلق، مو نقل «الفرق عن وضعية الراحة».**
        #
        # جرّبت طريقة الفرق (`restSrc⁻¹ · restRig`) وقِست النتيجة
        # بالدرجات على إطار مشي واحد:
        #   · الفخذ  **٦.٨°** فرق  → مقبول
        #   · الذراع **٣٧.٤°** فرق → الذراع تبقى مفتوحة
        #   · الساعد **٥٣.٤°** فرق → مكسور
        # والسبب **رياضي مو عيب تنفيذ**: طريقة الفرق تحافظ على
        # **طابع وضعية راحة الهدف**، وراحة هذا المجسّم أذرعها مفتوحة
        # **٥٩.٢°** مقابل وقفة T بالمصدر — ففرق الـ٣١° يبقى مضافاً
        # على كل إطار. ومقاس بالصورة كذلك: الأذرع مفتوحة مثل تمثال.
        #
        # والهيكلان **كلاهما محور العظمة فيه `Y`** (مفحوص)، فنسخ
        # الاتجاه المطلق يعطي **نفس وضعية المصدر بالضبط**.
        # ⚠️ وثمنه المحتمل **لَفّة الكف** (roll) — والكف بحجم ٩٦
        # بكسل ما يبيّن، والوضعية الصحيحة تبيّن فوراً.
